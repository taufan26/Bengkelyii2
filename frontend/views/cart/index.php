        ...
        <table class="table table-hover">
            ...
            <?php foreach ($items as $item): ?>
                <tr data-id="<?php echo $item['id'] ?>" data-url="<?php echo \yii\helpers\Url::to(['/cart/change-quantity']) ?>">
                    ...
                    <td>
                        <img src="<?php echo \common\models\Barang::formatImageUrl($item['image']) ?>"
                             style="width: 50px;"
                             alt="<?php echo $item['nama'] ?>">
                    </td>
                    <td><?php echo Yii::$app->formatter->asCurrency($item['harga']) ?></td>
                    <td>
                        <input type="number" min="1" class="form-control item-quantity" style="width: 60px" value="<?php echo $item['quantity'] ?>">
                    </td>
                    <td><?php echo Yii::$app->formatter->asCurrency($item['total_harga']) ?></td>
                    <td>
